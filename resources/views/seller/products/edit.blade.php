{{-- /opt/lampp/htdocs/gestion_des_encheres/resources/views/seller/products/edit.blade.php --}}
@extends('layouts.app')
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
            <div class="card-header bg-gradient-theme text-white rounded-top-4">
                <h5 class="mb-0 fw-bold">
                    <i class="material-symbols-rounded align-middle me-2">edit</i> Modifier le produit : {{ $product->nom }}
                </h5>
            </div>
                ...
                <form method="POST" action="{{ route('seller.products.update', $product) }}" enctype="multipart/form-data">
                    ...
                    <div class="row g-3">
                        <div class="col-md-12">
                            ...
                            <input type="text" name="nom" class="form-control @error('nom') is-invalid @enderror rounded-3" required value="{{ old('nom', $product->nom) }}">
                            @error('nom')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>

                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Marque</label>
                            <input type="text" name="marque" class="form-control @error('marque') is-invalid @enderror rounded-3" value="{{ old('marque', $product->marque) }}">
                            @error('marque')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Modèle</label>
                            <input type="text" name="modele" class="form-control @error('modele') is-invalid @enderror rounded-3" value="{{ old('modele', $product->modele) }}">
                            @error('modele')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>

                        <div class="col-md-12">
                            <label class="form-label fw-semibold">Description</label>
                            <textarea name="description" rows="3" class="form-control @error('description') is-invalid @enderror rounded-3">{{ old('description', $product->description) }}</textarea>
                            @error('description')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>

                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Catégorie <span class="text-danger">*</span></label>
                            <select name="categorie_id" id="categorie_id" class="form-select @error('categorie_id') is-invalid @enderror rounded-3" required>
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="etat" class="form-select @error('etat') is-invalid @enderror rounded-3" required>
                        ...
                        <!-- Upload zone for new photos -->
                        <div class="col-md-12">
                            <label class="form-label fw-semibold">Ajouter de nouvelles photos</label>
                            <div class="upload-zone rounded-3 p-4 text-center bg-light border-dashed" onclick="document.getElementById('photos-input').click()">
                                <i class="material-symbols-rounded mb-2" style="font-size: 48px; color: #cbd5e0;">add_photo_alternate</i>
                                ...
                                <input type="file" id="photos-input" name="photos[]" class="d-none" multiple accept="image/*" onchange="updateFileList(this)">
                            </div>
                            <div id="new-file-list" class="mt-3 d-flex flex-wrap gap-2"></div>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('seller.products.index') }}" class="btn btn-outline-secondary rounded-3 px-4">Annuler</a>
                        <button type="submit" class="btn btn-gradient rounded-3 px-4">
                            <i class="material-symbols-rounded align-middle me-1">save</i> Mettre à jour
                        </button>
                    </div>
                </form>
            </div>
